...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    box_wh = K.exp(feats[..., 2:4]) * anchors_tensor / K.cast(input_shape[::-1], K.dtype(feats))   # 报错
...
def yolo_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape):
    ...
    box_xy, box_wh, box_confidence, box_class_probs = yolo_head(feats,    # 报错
...
def yolo_eval(yolo_outputs,
    ...
    for l in range(num_layers):
        _boxes, _box_scores = yolo_boxes_and_scores(yolo_outputs[l],      #  报错
